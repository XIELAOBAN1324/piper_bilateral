...
"""
Piper Phase 2B-1: timing-correct MIT teleoperation baseline
...
Optional software zero correction, degrees:
    python tests/step2b1_mit_teleop.py --live \
...
class LeaderReader:
    """
    Publish only complete 6-joint snapshots.

    0x155, 0x156 and 0x157 update joint pairs independently.  The original
    v2 exposed self.q directly, so the control loop could theoretically read
    J1/J2 from a new CAN cycle while J3..J6 still belonged to the previous
    one.

    Here a public snapshot is updated only after all three pair frames have
    ...
    def __init__(self, channel):
        ...
        self._working_q = [None] * 6
        self._pair_ts = [0.0, 0.0, 0.0]
        self._dirty_mask = 0

        self._snapshot_q = None
        self._snapshot_seq = 0

        # Latest pair timestamp is used as the snapshot sample timestamp.
        # Oldest pair timestamp is used for watchdog age, conservatively.
        self._snapshot_sample_ts = 0.0
        self._snapshot_oldest_ts = 0.0
    ...
    def _publish_if_complete(self):
        if self._dirty_mask != 0b111:
            return

        if any(x is None for x in self._working_q):
            return

        self._snapshot_q = list(self._working_q)
        self._snapshot_seq += 1
        self._snapshot_sample_ts = max(self._pair_ts)
        self._snapshot_oldest_ts = min(self._pair_ts)

        # Require a fresh frame from every pair before the next publication.
        self._dirty_mask = 0
    ...
    def _loop(self):
        while self.running:
            ...
            with self.lock:
                if can_id == CAN_J12:
                    self._working_q[0] = self.decode_joint(data[0:4])
                    self._working_q[1] = self.decode_joint(data[4:8])
                    self._pair_ts[0] = now
                    self._dirty_mask |= 0b001

                elif can_id == CAN_J34:
                    self._working_q[2] = self.decode_joint(data[0:4])
                    self._working_q[3] = self.decode_joint(data[4:8])
                    self._pair_ts[1] = now
                    self._dirty_mask |= 0b010

                elif can_id == CAN_J56:
                    self._working_q[4] = self.decode_joint(data[0:4])
                    self._working_q[5] = self.decode_joint(data[4:8])
                    self._pair_ts[2] = now
                    self._dirty_mask |= 0b100
    ...
    def get(self):
        ...
        with self.lock:
            if self._snapshot_q is None:
                return None, float("inf"), 0, 0.0

            age = now - self._snapshot_oldest_ts

            return (
                list(self._snapshot_q),
                age,
                self._snapshot_seq,
                self._snapshot_sample_ts,
            )
...
class LeaderVelocityEstimator:
    ...
    def update(self, q, sample_ts):
        ...
        if dt <= 0.0005 or dt > LEADER_TIMEOUT:
            self.last_q = list(q)
...
def main():
    ...
    try:
        print("=" * 78)
        print("Piper Phase 2B-1 MIT Teleoperation")
        print("=" * 78)
        ...
        input(
            "Move LEFT to the desired reference pose, "
            "then press Enter... "
        )
        ...
        # ----------------------------------------------------
        # Wait for a coherent fresh leader snapshot
        # ----------------------------------------------------
        print("[4] Wait for fresh complete leader state")

        wait_deadline = time.monotonic() + 3.0

        while True:
            qm, age, leader_seq, sample_ts = leader.get()

            if (
                qm is not None
                and age < LEADER_TIMEOUT
            ):
                break

            if time.monotonic() > wait_deadline:
                raise RuntimeError(
                    "no fresh complete leader state"
                )
        ...
        while True:
            ...
            # --------------------------------------------
            # Velocity update only when a new coherent
            # leader snapshot has actually arrived.
            # --------------------------------------------
            leader_sample_dt = 0.0

            if leader_seq != last_leader_seq:
                (
                    leader_sample_dt,
                    dq_raw,
                    dq_filt,
            ...
            # --------------------------------------------
            # Keep the v2 RELATIVE mapping in Phase 2B-1.
            #
            # This intentionally preserves startup pose
            # offset. We diagnose that separately rather
