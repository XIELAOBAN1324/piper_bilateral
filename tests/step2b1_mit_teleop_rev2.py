...
def main():
    ...
    try:
        print("=" * 78)
        print("Piper Phase 2B-1 MIT Teleoperation (rev2)")
        print("=" * 78)
        ...
        while True:
            ...
            if now - last_diag_print > 0.5:
                ...
                print(
                    "  leader counts J12/J34/J56={}  ages_ms={}  thread_error={}"
                    .format(
                        leader_diag["counts"],
                        ages_ms,
            ...
            if now > wait_deadline:
                raise RuntimeError(
                    "leader frames not ready; counts={} ages_ms={} error={}"
                    .format(
                        leader_diag["counts"],
                        [
                            round(x * 1000.0, 1)
        ...
        while True:
            ...
            # --------------------------------------------
            # Velocity update from the latest 6-joint cache.
            #
            # The three CAN pair frames are asynchronous, so raw CAN seq
            # increments are not treated as a six-joint sample clock.
            # Update once per control iteration using real monotonic time.
            # --------------------------------------------
            velocity_now = time.monotonic()
            leader_sample_dt = velocity_now - previous_velocity_ts
